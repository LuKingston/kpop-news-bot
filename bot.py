...
async def start_cmd(message: types.Message):
    async with aiosqlite.connect("users.db") as db:
        cursor = await db.execute(
            "SELECT group_name FROM subscriptions WHERE user_id = ?", (message.from_user.id,)
        )
        subs = await cursor.fetchall()
        subs_list = ', '.join([g[0] for g in subs]) or "нет"
        await message.answer(
            f"Ты подписан(а) на: {subs_list}\n\nВыбери K-pop группы, о которых хочешь получать новости:",
...
async def forward_handler(message: types.Message):
    ...
    matched_groups = [
        g for g in GROUPS if g.upper().replace(" ", "") in [h.upper() for h in hashtags]
    ]
